    ...
    public static void main(String[] args) {
        ...
        while (true) { //this while loops until user exits the program!
            ...
            switch ((command.toUpperCase())) {
                ...
                case "107":
                case "LPD":
                    getFromTxt(); //https://youtu.be/pBS_I88JMXw(reading and writing java objects to a file)
                    ArrayList<FoodQueue> retreivedCashiers = getFromTxt();
                    if(retreivedCashiers != null){
                        cashiers.setCashiers(retreivedCashiers);
...
//        System.out.println(reqBurgers);(for debugging purpose)
    ...
    public static void saveToTxt(Cashiers cashiers) {//Modified method for store data into a file(https://www.geeksforgeeks.org/fileoutputstream-in-java/)
        String fileNameCashiers = "cashiers.txt"; // Specify the filename or path
        String fileNameBurger = "burgers.txt";
        try {
            FileOutputStream fOutCashiers = new FileOutputStream(fileNameCashiers);
            ObjectOutputStream oOutCashier =new ObjectOutputStream(fOutCashiers);
            oOutCashier.writeObject(cashiers.getCashiers());
            oOutCashier.close();
            fOutCashiers.close();

            FileOutputStream fOutBurgers = new FileOutputStream(fileNameBurger);
            ObjectOutputStream oOutBurgers =new ObjectOutputStream(fOutBurgers);
            oOutBurgers.writeObject(cashiers.getBurgers());
            oOutBurgers.close();
            fOutBurgers.close();
    ...
    public static ArrayList<FoodQueue> getFromTxt() {//Modified method for get data from a file(https://www.geeksforgeeks.org/fileoutputstream-in-java/)
